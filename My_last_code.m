...
Tref_params = [15, 15, 15]; % Reference temperature (°C) - Using a single value for simplicity
...
PowerCapacity_values = zeros(num_simulations, 1);
Rf_values = zeros(num_simulations, 1);
eta_c_values = zeros(num_simulations, 1);
F_values = zeros(num_simulations, 1);

% Define distribution parameters
area_params = [200, 225, 250]; % Area (km^2)
thickness_params = [2, 2.5, 3]; % Thickness (km)
TR_uniform_params = [150, 200, 250]; % Reservoir temperature (°C)
rho_r_params = [2350, 2700, 3000]; % Rock Density (kg/m^3)
phi_params = [0.01, 0.05, 0.1]; % Porosity (fraction)

Rf_params = [3, 11, 17]; % Recovery factor (%)
...
for i = 1:num_simulations
    ...
    PowerCapacity = (qT * Rf * eta_c) / (F * plant_life_seconds);
    PowerCapacity_values(i) = PowerCapacity / 1e9;
end
...
mean_PowerCapacity = mean(PowerCapacity_values);
std_PowerCapacity = std(PowerCapacity_values);
min_PowerCapacity = min(PowerCapacity_values);
max_PowerCapacity = max(PowerCapacity_values);
...
fprintf('Electrical Power Harvested (Power Capacity Potential):\n');
fprintf('Mean: %.2f GWe\n', mean_PowerCapacity);
fprintf('Standard Deviation: %.2f MWe\n', std_PowerCapacity);
fprintf('Minimum: %.2f GWe\n', min_PowerCapacity);
fprintf('Maximum: %.2f GWe\n', max_PowerCapacity);
...
save('simulation_results.mat', 'qR_values', 'qF_values', 'PowerCapacity_values', 'num_simulations');
...
figure;
histogram(PowerCapacity_values, num_bins, 'Normalization', 'pdf');
...
xticks = linspace(min(PowerCapacity_values), max(PowerCapacity_values), 11);
xticklabels = arrayfun(@(x) sprintf('%.2f', x), xticks, 'UniformOutput', false);
set(gca, 'XTick', xticks, 'XTickLabel', xticklabels);

% Calculate 90% confidence interval
sorted_PowerCapacity_values = sort(PowerCapacity_values);
lower_bound = sorted_PowerCapacity_values(round(0.05 * num_simulations));
upper_bound = sorted_PowerCapacity_values(round(0.95 * num_simulations));
...
[counts, edges] = histcounts(PowerCapacity_values, num_bins, 'Normalization', 'pdf');
[~, max_idx] = max(counts);
...
[cdf_values, cdf_edges] = histcounts(PowerCapacity_values, num_bins, 'Normalization', 'cdf');
...
p10 = prctile(PowerCapacity_values, 10);
p50 = prctile(PowerCapacity_values, 50);
p90 = prctile(PowerCapacity_values, 90);
...
% Plotting the total thermal energy (qT) pdf and showing 90% confidence interval

% Plot the frequency distribution of total thermal energy
figure;
histogram(qT_values, num_bins, 'Normalization', 'pdf');
xlabel('Total Thermal Energy (J)');
ylabel('Probability Density');
title('Frequency Distribution of Total Thermal Energy');
grid on;

% Set x-axis intervals
xticks = linspace(min(qT_values), max(qT_values), 11);
xticklabels = arrayfun(@(x) sprintf('%.2e', x), xticks, 'UniformOutput', false);
set(gca, 'XTick', xticks, 'XTickLabel', xticklabels);

% Calculate 90% confidence interval
sorted_qT_values = sort(qT_values);
lower_bound_qT = sorted_qT_values(round(0.05 * num_simulations));
upper_bound_qT = sorted_qT_values(round(0.95 * num_simulations));

hold on;
xline(lower_bound_qT, 'r--', 'Lower 90% CI');
xline(upper_bound_qT, 'r--', 'Upper 90% CI');

% Display confidence interval on the plot
text(lower_bound_qT, 0.8 * max(ylim), sprintf('%.2e', lower_bound_qT), 'Color', 'red');
text(upper_bound_qT, 0.8 * max(ylim), sprintf('%.2e', upper_bound_qT), 'Color', 'red');

% Determine the most likely interval (mode of the distribution)
[counts_qT, edges_qT] = histcounts(qT_values, num_bins, 'Normalization', 'pdf');
[~, max_idx_qT] = max(counts_qT);
most_likely_interval_qT = [edges_qT(max_idx_qT), edges_qT(max_idx_qT + 1)];

% Highlight the most likely interval on the plot
patch([most_likely_interval_qT(1), most_likely_interval_qT(2), most_likely_interval_qT(2), most_likely_interval_qT(1)], ...
      [0, 0, max(counts_qT), max(counts_qT)], 'yellow', 'FaceAlpha', 0.3, 'EdgeColor', 'none');

legend('Energy Distribution', 'Lower 90% CI', 'Upper 90% CI', 'Most Likely Interval');
hold off;